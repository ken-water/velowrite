...
A guided walkthrough of the VeloWrite workflow: open a complex Markdown document with tables, code, Mermaid, KaTeX math, and tabbed Python/Bash/Java/JavaScript code samples; switch between full-screen write mode, split preview, and full-screen preview; export Markdown or HTML; then move serious local-first writing to the desktop preview.
...
- `/demo` presents the guided interactive Product Hunt walkthrough with complex Markdown, math rendering, tabbed code samples, and Write/Split/Preview mode switching.
